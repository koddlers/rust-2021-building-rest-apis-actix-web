...
pub fn get_all_flight_plans() -> Result<Option<Vec<FlightPlan>>, Box<dyn Error>> {
    let mut flight_plan_list = Vec::new();
    let connection = get_database_connection()?;
...
pub fn insert_flight_plan(flight_plan: FlightPlan) -> Result<(), Box<dyn Error>> {
    ...
    let mut statement = connection.prepare("INSERT INTO flight_plan (
                flight_plan_id, altitude, airspeed, aircraft_identification,
...
pub fn update_flight_plan(flight_plan: FlightPlan) -> Result<bool, Box<dyn Error>> {
    ...
    let mut statement = connection.prepare("UPDATE flight_plan SET altitude = ?,
